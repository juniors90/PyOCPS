...
p = np.ceil(np.random.random(No_pop, NBus - 1) * No_Cap_Type)  #* Initial popoulation
...
for i in range(1, np.size(p, axis=0) + 1):
    ...
    np.array(Load)[:, 0] = LoadOffPeak - (np.array(pop)[i, :]).conj().transpose()
    np.array(Load)[:, 1] = LoadMedium - (np.array(pop)[i, :]).conj().transpose()
    ...
    for il in range(1 ,NLoadLevel+1):
        ...
        PenaltyVoltageL[i-1, il-1] = pen_v(Vbus) # Calculating amount of penalties
